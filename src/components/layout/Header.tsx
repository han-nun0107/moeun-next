...
const Header = () => {
  ...
  return (
    <header
      className="fixed top-0 left-0 flex h-[90px] w-full items-center justify-around border-b border-gray-300 bg-white"
            ...
            <ul className="flex list-none gap-15">
              {NAV_ITEMS.map((item) => (
                <li key={item.path}>
                  <Link
                    href={item.path}
                    className="text-black-200 text-lg font-semibold"
                  >
                    {item.label}
                  </Link>
                </li>
